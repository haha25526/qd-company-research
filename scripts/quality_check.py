...
def check_cognitive_models(content: str) -> tuple[bool, str]:
    ...
    return passed, f"思维模型: {model_cnt} 个 {'OK' if passed else 'FAIL'}"
...
def check_verification_status(content: str) -> tuple[bool, str]:
    ...
    passed = ratio >= 0.5
    return passed, f"验证覆盖率: {cross}跨域/{pred}预测/{excl}排他 {'OK' if passed else 'FAIL'}"
...
def check_uncertainty_marking(content: str) -> tuple[bool, str]:
    ...
    return passed, f"不确定性标注: {'OK' if passed else 'FAIL'}"
