...
当前 CLI 入口是**研究文件处理层**，用于处理已有研究文件。流程为：
1. 确认公司身份和研究目录
...
如果需要补齐研究，产品流程设计为“主管 + 六路研究员”的并行模式，而不是串行跑完 A/B/C/D/E/F：

1. 主管同时启动 A 财务、B 业务、C 战略、D 领导思维、E 批判视角、F 文化符号六路研究。
2. 每一路使用独立上下文，只共享公司身份、研究范围和本路任务说明。
3. 每一路完成后落盘 raw、cognitive 和 context_card，再单独向用户汇报摘要、来源质量、不确定性和产物路径，并等待确认。
4. 用户可以只要求某一路返工；已确认的其他研究流不受影响。
5. 六路全部确认后，再进入 Review 汇总、认知提取和质量检查。

本地 `run_improved.py` 负责检查、汇总、验证和生成 Skill；并行研究是 AI 执行层的协作协议。真正的能力边界在于公开信息是否可得、来源质量是否足够、以及当前执行环境是否支持并行子任务。

### 上下文管理

六路研究不应共享同一个大上下文池。推荐文件约定：
...
`context_card` 是跨阶段交接物，记录已确认结论、关键证据、不确定性、反例冲突和给 Review 阶段的压缩摘要。汇总阶段优先读取 `*_context_card.md` 和 `*_cognitive.md`，只在需要追溯证据时回看 raw 文件。
